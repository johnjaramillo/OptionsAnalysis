...
def get_days_to_expiration(expiration_date_str, buy_date_obj):
    expiration_date = datetime.strptime(expiration_date_str, "%Y-%m-%d")
    return (expiration_date - buy_date_obj).days
...
def main():
    ...
    symbol = st.text_input("Stock Symbol (e.g., FUBO)").upper()
    ...
    if symbol:
        try:
            ...
            st.error("Could not fetch expiration dates")
            return
    else:
    ...
    strike = st.number_input("Strike Price", min_value=0.01, step=0.01, format="%.2f")

    moneyness_pct = st.number_input("Moneyness % (e.g., -12 for -12%)", step=0.1, format="%.2f")

    iv = st.number_input("Implied Volatility (%)", min_value=0.0, step=0.1, format="%.2f")

    delta = st.number_input("Delta (0 to 1)", min_value=0.0, max_value=1.0, step=0.01, format="%.4f")

    premium = st.number_input("Premium ($)", min_value=0.0, step=0.01, format="%.4f")
    ...
    if st.button("Analyze Option"):
        ...
        with st.spinner("Fetching data and analyzing..."):
            ...
            # Fetch option chain info
            try:
                chain = ticker.option_chain(expiration)
                df = chain.calls if option_type == "call" else chain.puts
                row = df[df['strike'] == strike]
                if row.empty:
                    st.error("Strike price not found in option chain for this expiration.")
                    return
